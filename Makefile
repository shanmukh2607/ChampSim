...
clean: 
	...
	$(RM) inc/ooo_cpu_modules.inc
	 ...
	 find branch/gl_double -name \*.o -delete
	 find branch/gl_double -name \*.d -delete
	 find btb/basic_btb -name \*.o -delete
	 find btb/basic_btb -name \*.d -delete

bin/champsim: $(patsubst %.cc,%.o,$(wildcard src/*.cc)) obj/repl_rreplacementDlru.a obj/pref_pprefetcherDno.a obj/pref_pprefetcherDno_instr.a obj/bpred_bbranchDgl_double.a obj/btb_bbtbDbasic_btb.a
...
branch/gl_double/%.o: CFLAGS += -Ibranch/gl_double
branch/gl_double/%.o: CXXFLAGS += -Ibranch/gl_double
branch/gl_double/%.o: CXXFLAGS +=  -Dinitialize_branch_predictor=bpred_bbranchDgl_double_initialize -Dlast_branch_result=bpred_bbranchDgl_double_last_result -Dpredict_branch=bpred_bbranchDgl_double_predict
obj/bpred_bbranchDgl_double.a: $(patsubst %.cc,%.o,$(wildcard branch/gl_double/*.cc)) $(patsubst %.c,%.o,$(wildcard branch/gl_double/*.c))
	@mkdir -p $(dir $@)
	ar -rcs $@ $^
...
-include $(wildcard branch/gl_double/*.d)
